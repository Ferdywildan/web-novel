                            ...
                            foreach ($member as $row) : ?>
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <a href="<?= base_url('member/delete/' . $row['id']); ?>" class="btn btn-sm btn-danger" onclick="return confirm('apakah anda yakin?')"><i class="fa fa-trash"></i></a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
...
<div class="modal fade" id="modal-default">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="form" action="<?= base_url('member/add') ?>" method="POST" enctype="multipart/form-data">
...
<script>
    $(document).ready(function() {
        $('#addBuku').on('click', function() {
            $('.modal-title').html('Tambah Data member');
            $('#form').attr('action', '<?= base_url('member/add') ?>');
            $('#no_reg').val('<?= $no_reg ?>');
            $('#nama_member').val('');
            $('#jenis_kelamin').val('');
            $('#password').val('');
        ...
        $(document).on('click', '.edit', function() {
            let id = $(this).data('id');
            $('.modal-title').html('Edit Data member');
            $('#form').attr('action', '<?= base_url('buku/update/') ?>' + id);
            $.ajax({
                url: '<?= base_url('member/getdata/'); ?>' + id,
                method: 'POST',
                dataType: 'JSON',
                data: {
                    id: id
